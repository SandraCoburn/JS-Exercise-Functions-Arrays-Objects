...
function sortCarInventory(inventory) {
  ...
  inventory.sort(function(a, b) {
    let invOne = a.car_model.toUpperCase();
    let invTwo = b.car_model.toUpperCase();
    if (invOne < invTwo) {
      return -1;
    }
    if (invOne > invTwo) {
...
function getGermanCars(inventory) {
  ...
  for (let i = 0; i < inventory.length; i++) {
    ...
    if (carNum.car_make === 'Audi' || carNum.car_make === 'Mercedes-Benz' || carNum.car_make === 'Volkswagen' || carNum.car_make === 'BMW') {
      germCar.push(carNum);
    }
  }
  return germCar;
